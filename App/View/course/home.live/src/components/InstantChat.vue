<template>
  <div class="instant-chat" ref="chat" :style="{height:height+'px'}" @scroll="chatSroll">
    <div class="instant-chat-pull-loading" v-if="isShowLoading"></div>
    <div class="instant-chat-pull-up" v-else-if="page < totalPage">上拉加载更多消息</div>
    <div v-for="(item,index) in chatData" :key="index">
      <div class="instant-chat-reward" v-if="item.content && item.content.msg_type === 'reward'">
        <div class="instant-chat-reward__text">
          {{item.membername}}打赏了
          <span>{{item.content.reward_number}}</span>元红包
        </div>
      </div>
      <div class="instant-chat-tips" v-else-if="item.content && item.content.msg_type === 'tips'">
        <div class="instant-chat-tips__text">{{item.content.msg}}</div>
      </div>
      <div v-else>
        <div class="instant-chat-time" v-if="item.adddate">{{item.adddate}}</div>
        <div class="instant-chat-item clearfix">
          <ks-image class="instant-chat-item__avatar" :src="item.avatar" type="user"/>
          <div class="instant-chat-item__name">
            {{item.membername}}
            <span
              class="instant-chat-item__level"
            ...
            >撤回</span>
            <span
              @click="$stop($event);replyMsg(item)"
              v-if="item.memberid && item.memberid != userid && item.chattype == 2"
            >回复</span>
          </div>
          <template v-if="item.content">
            <template v-if="item.content.msg_action === 'reply'">
              <div class="instant-chat-item__reply">
                <div class="instant-chat-item__reply-content" v-html="item.content.msg"></div>
                <div class="instant-chat-item__reply-source" v-if="item.content.reply_source">
                  <div
            ...
            <template v-else>
              <div
                class="instant-chat-item__interact"
                v-if="item.content.msg_type === 'text'"
                v-html="item.content.msg"
              ></div>
              <div class="instant-chat-item__interact" v-else-if="item.content.msg_type === 'img'">
                <img
                  class="preview-img"
              ...
              <div
                class="instant-chat-item__audio"
          ...
          <template v-else>
            <div class="instant-chat-item__interact instant-chat-item__interact--error">不支持该内容格式</div>
          </template>
        </div>
      </div>
    </div>
    <div
      class="instant-chat-msg"
      @click="scrollToLower"
      :style="{width:chatWidth+'px',top:chatHeight-32+'px',left:chatOffsetLeft+'px'}"
...
export default {
  ...
  methods: {
    ...
    openPrevImg(id) {
      ...
      // this.$preview.open(prevIndex, this.prevImgList);
    },
    /**
...
.instant-chat-pull-loading {
  height: 25px;
  line-height: 25px;
  overflow: hidden;
  background: url(../assets/instant-chat/loading.gif) no-repeat 50% 50%;
  background-size: auto 25px;
}
.instant-chat-pull-up {
  text-align: center;
  height: 25px;
  line-height: 25px;
  overflow: hidden;
...
.instant-chat-item__name span {
  margin-left: 10px;
  color: #37a7ed;
  font-size: 12px;
  cursor: pointer;
}
.instant-chat-item__name .instant-chat-item__level {
  color: #989da7;
  cursor: default;
}
.instant-chat-item__interact {
  float: left;
  background: #fff;
  padding: 7px 10px;
  border-radius: 5px;
...
.instant-chat-item__interact img {
  max-width: 260px;
  max-height: 260px;
  display: block;
  padding: 3px 0;
...
.instant-chat-item__reply-content {
  border-bottom: 1px solid #eee;
  padding-bottom: 5px;
}
.instant-chat-item__reply-source {
  padding-top: 5px;
}
.instant-chat-item__audio {
  width: 180px;
  height: 36px;
  background-color: #fff;
  background-image: url(../assets/instant-chat/audiolive_icon_audiodoc.png);
...
.instant-chat-item__audio.playing {
  background-image: url(../assets/instant-chat/audiolive_icon_audiodoc.gif);
}
.instant-chat-item__audio span {
  float: right;
  padding-right: 10px;
  color: #3a3d89;
}
.instant-chat-item__audio i {
  width: 7px;
  height: 7px;
  position: absolute;
  right: -2px;
...
.instant-chat-reward__text sapn {
  color: #e64a3b;
}
.instant-chat-tips {
  text-align: center;
