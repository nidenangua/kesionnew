...
<form target="hidframe"  method="post" action="<?php if(isset($value)){echo M_URL($this->AppName.'/Index','doeditKpoint',$value['id'],GP('')); }else{echo M_URL($this->AppName.'/Index','doaddKpoint','',GP(''));} ?>"  id="myform">
	<div class="bg-white ks-wbox bRadius10 ks-sBox">
        	...
        	<div class="content_bix_wrp" >
	            ...
	            <div class="form-row">
	              		<span class="name"><i class="required">*</i>知识点</span>
	              		<div class="form-limit" limit="50">
	              			<input type="text" class="form-textbox" name="title" value="<?php if(!empty($value)) echo $value['title'] ?>" />
	              		</div>
	            </div>
	            <div class="form-row">
	              		<span class="name"><i class="required">*</i>讨论主题</span>
	              		<div class="form-limit" limit="50">
	              			<textarea type="text" class="form-textarea" name="intro"  />
	              			   <?php if(!empty($value)) echo $value['intro'] ?>
	              			</textarea>
	              		</div>
	            </div>
			   
			</div>
       	 <div id="ksBoxFooter clearfix">
					<div class="form-footer">
						<button type="button" onClick="do_reg()" class="ks-bt bt-pue mr30"><?php if(!empty($userinfo)){echo "确认修改";}else{echo "确认添加";} ?></button>
