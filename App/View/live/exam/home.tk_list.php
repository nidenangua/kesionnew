...
<style>
.inputText {height: 32px;line-height: 32px;}
.orz{margin: 15px 20px;}
...
<body class="ks-wrap">
     ...
     <div id="ksBoxHead">
        <select class="inputText bRadius5 w150 c-79 mr30 fl" onchange="getval(this,'<?php echo GP(''); ?>','<?php echo M_URL($this->AppName.'/Index','tkList',$typeid); ?>','courseid','<?php echo URL_MODEL;?>')">    
    ...
    <div class="search-box orz clearfix" >题型
        <select class="inputText bRadius5 w150 c-79 mr30 fl" onchange="getval(this,'<?php echo GP(''); ?>','<?php echo M_URL($this->AppName.'/Index','tkList',$typeid); ?>','tktype','<?php echo URL_MODEL;?>')">
                    <option value="0" <?php if(isset($tktype))if($tktype == 0) echo "selected"; ?>></option>
                    ...
                    <option value="6" <?php if(isset($tktype))if($tktype == 6) echo "selected"; ?>>材料题</option>
        </select>
      </div>    
    <div class="search-box orz clearfix" >难度
      ...
      <div class="search-box orz clearfix" >类型
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0"
        ...
        <tr>
            ...
            <td class="alignCenter hLh40 pt0 pb0 fsize14 c-92"><?php echo $v['inputer'];?></td>
        </tr>
       <?php }?>
        </tbody>
...
function getTK(obj){
	...
	if(!selectTk){
		...
		ab.push(b);
		var tk = ab.join(",");
		$.cookie('selectTk',tk,{ expires: 7, path: '/' });
	}else{
		ab = selectTk.split(",");
		if(a==false){
