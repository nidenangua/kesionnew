...
<body>
	  ...
	  <div class="fr ks-top-search">
				<label class="mr0">
			   	<select class="w150 inputText bRadius5" id="keytype">
					<option value="1" <?php if($keytype==1){echo 'selected="selected"';} ?>>学员名字</option>
					<option value="2" <?php if($keytype==2){echo 'selected="selected"';} ?>>学员性别</option>
			  	</select>
				</label>
				<div class="ks-top-search fr">
					<input type="text" class="sc-text" name="searchText" id="keyword"  value="<?php if(isset($keyword)) echo $keyword; ?>">
	                <input type="button" class="sc-button" onClick="searchByClass('<?php echo GP(''); ?>','<?php echo M_URL($this->AppName.'/Index','signStudent',$classroomid); ?>','<?php echo URL_MODEL;?>')" name="searchBtn" value="搜索" >
				</div>
			</div>
		</div>   
	...
	<form target="hidframe"  action="<?php  echo M_URL($this->AppName.'/Index','sign',$classroomid,GP('p-'.$page->now_page.',courseid-'.$courseid)) ?>" method="post" id="myform">
            ...
            <tbody>
             <?php foreach($values as $k => $v){ ?>
             	<tr>
            		...
            		<input type="checkbox" class="select-check" name="student[]" value="<?php echo Field($v['userid']);?>"></td>
             		<td width="15%"><?php echo Field($v['name']);?></td>
             		<td width="10%"><?php echo Field($v['Sex']);?></td>
             		<td width="13%"><?php echo Field($v['QQ']);?></td>
             		<td><?php echo Field(date("Y-m-d H:i:s",$v['time']));?></td>
             		<td>
                </td>
             	</tr>
             <?php }?>
	...
	<div >
		...
		<div class="clearfix mr30">
		    <div class="rightTd alignLife fl">
		        ...
		        <input type="button" class="ks-bt bt-pue"  onclick="submitform()"   value="签到" name="confirm" />
