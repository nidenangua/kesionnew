...
<head>
	...
	<body>
		<div class="verm-box" id="incomedetails" v-cloak>
			...
			<div class="change-wrap" v-if="campusArr.length>0">
			...
			methods:{
				getdata:function(type){
					...
					newData.keyword = this.search;
		 			$post(that.jsonUrl+'?act=Microportal.mpaylog',newData,function(res){
		 				that.income = res.mpaylog.money; 
		 				//如果没有数据的话
		 				if(res.mpaylog.page.total_pages==0){
		 					that.loadMoreFlag = false;
		 					return false
		 				}
		 				var datas = res.mpaylog.list;
		 				console.log(datas);
