...
<div class="bRadius10 ks-wbox">
	...
	<div id="ksBoxContent">
            ...
            <table width="100%" cellpadding="0" cellspacing="0" border="0"  class="ks-table">
                ...
                <tbody>
                <?php for($i=0;$i<$array[3];$i++){ ?>
                    <tr>
                        <td class="form-in">
                            <input type="checkbox" class="select-check" name="id[]" value="<?php echo $v['id'] ?>">
                        </td>
                        <td class="w200"><?php echo Field($score[$i]);?></td>
                         
                        <td class="w200"><?php echo Field($money[$i]);?></td>
        ...
        <!--<div><?php echo "活动时间". Field(date("Y-m-d",$num['startime']))."/".Field(date("Y-m-d",$num['endtime']));?>   </div>
        <div><?php echo "开奖时间". Field(date("Y-m-d",$num['opentime']));?>   </div>
         <div><?php echo "红包策略:  ". $num['strategy'];?>   </div>
	    -->
    </div>
