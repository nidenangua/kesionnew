				...
				<div class="ibox-title">
					<div class="clearfix" style="float: left;">
						...
						<div style="float: left;" class="shaixuan addCoupon clearfix">
								<a class="desc" href="<?php echo M_URL($this->AppName.'/Index','couponadd','',GP('p-'.$page->now_page));?>"><i class="iconfont icon-jiahao">新增优惠券</a>
							</div>
					</div>
			     ...
			     <option value="1">用户名</option>
							...
							<span class="inputText bRadius5 fl" style="width: 150px;cursor: pointer;line-height:34px;text-align: center;">
								优惠券名称
							</span>
							...
							<ul class="H5-gameScreen">
								...
								<li>
									...
									<div>
										...
										<a href="<?php echo M_URL($this->AppName.'/Index','index','',GP('p-'.$page->now_page.',status-1'));?>" class='<?php if($status==1) echo " active" ;?>'>可用</a>
										<a href="<?php echo M_URL($this->AppName.'/Index','index','',GP('p-'.$page->now_page.',status-2'));?>" class='<?php if($status==2) echo " active" ;?>'>停用</a>
									</div>
								</li>
							</ul>
							...
							<table class="ks-table" id="tables">
								...
								<tbody>
									...
									<tr>
										...
										<td>
											<?php if($v['endtime']>time()){echo date('Y-m-d H:i:s',$v['startime']).'-</br>'.date('Y-m-d H:i:s',$v['endtime']);}else{echo '<span style="color:red">已过期</span>';}?>
										</td>
										<td>
											<span onclick="Confirm('是否决定<?php if($v["status"] == 1){echo "停用";}else{echo "启用";} ?>','<?php echo M_URL($this->AppName.'/Index','changeStatus',$v['id'],GP('p-'.$page->now_page.',status-'.$v['status'])); ?>')"><?php if($v['status'] == 1){echo "<span style='color:green'>可用</span>";}else{echo "<span style='color:red'>停用</span>";} ?></span>
										</td>
										<td>
											<?php echo Field($v['descript']);?>
										</td>
										<td>
											<a href="<?php echo M_URL($this->AppName.'/Index','showDetail','',GP('id-'.$v['id'])); ?>">查看</a>
										</td>
										<td>
											<a href="<?php echo M_URL($this->AppName.'/Index','couponadd',$v['id'],GP('p-'.$page->now_page)); ?>"><i class='edit2017'></i></a>
										</td>
										<td>
											<div class="operate">
												<a href="javascript:void(0);" onclick="Confirm('<?php if($v['endtime']>time()){echo "该优惠券不可删除";}else{echo "确定删除该优惠券？该操作不可逆";} ?>','<?php if($v['endtime']>time()){ echo ''; }else{ echo M_URL($this->AppName.'/Index','delete',$v['id'],GP('p-'.$page->now_page));} ?>')">
							<i class='delete2017'></i></a>
											</div>
										</td>
										<td>
											<a href="javascript:void(0);" onclick="Confirm('<?php if($v["status"] == 0||$v['endtime']<time()){echo "该优惠券不可用";}else{echo "是否发放该优惠券";} ?>','<?php if($v["status"] == 0||$v['endtime']<time()){ echo ''; }else{ echo M_URL($this->AppName.'/Index','showgrant','',GP('type-2,maxid-'.$v['id']));} ?>')">发放</a>
										</td>
										<td>
											<a href="javascript:void(0);" onclick="Confirm('<?php if($v["status"] == 0||$v['endtime']<time()){echo "该优惠券不可生成";}else{echo "是否生成该优惠券";} ?>','<?php if($v["status"] == 0||$v['endtime']<time()){ echo ''; }else{ echo M_URL($this->AppName.'/Index','build',$v['id'],GP('p-'.$page->now_page));} ?>')">生成</a>
										</td>
									</tr>
									<?php }?>
								</tbody>
