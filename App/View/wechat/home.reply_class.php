		  	...
		  	<li class="clearfix posRe">
			  	<div class="Suin">
			  		<div class="SuinImg">
			  			...
			  			<input class="SuninRa" type="radio" id="less<?php echo $v['classid'];?>"  name="sucaibox" value="<?php echo $v['classid'];?>">
			  		...
			  		<div class="SuinMark mt5">
				  		...
				  		<div class="hLh30 Suinintro" style="display:none;"><?php echo $v['classintro'];?></div>
				  		<input type="hidden" class="Suinauthor" name=""  value="<?php echo $v['name'];?>"/>
				  		<input type="hidden" class="Suintext" name=""  value='<?php echo Img($v['detailintro'],2);?>'/>
				  		<input type="hidden" class="Suinlinkurl" name=""  value="<?php echo 'http://'.APP_DOMAIN.'/class/view/'.$v["classid"]; ?>"/>
	...
	$(".SuinPost").click(function(){
		...
		$('.SuinHover').each(function(){
			if($(this).css("display") == 'inline'){
				state++;
				data='<div class="Suin"><div class="SuinImg"><input class="SuninRa" type="hidden" required name="classid" value="'+SuinId+'">';
