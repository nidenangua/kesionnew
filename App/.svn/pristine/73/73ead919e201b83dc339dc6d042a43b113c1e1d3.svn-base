			...
			<!--START HEAD-->
				<div class="clearfix">
					<div class="form-tip clearfix">
						...
						<span class="form-tip-word fl">班级公告</span>
						<span class="fr">
							<a class="ks-bt bt-def mr10" href="<?php echo M_URL($this->AppName.'/Index','addNotice',$classroomid,GP('')) ?>" id="addNotice">
							<i class="iconfont icon-jiahao"></i>公告</a>
						...
						<form target="hidframe"  class="fr">
							<div class="ks-top-search">
								...
								<input type="submit" class="sc-button"  onClick="searchByClass('<?php echo GP(''); ?>','<?php echo M_URL($this->AppName.'/Index','showNotice',$classroomid); ?>','<?php echo URL_MODEL;?>');" name="searchBtn" value="搜索" >
							</div>
						</form>
						...
						<div class="stuConBox">
							<div class="studentBox" style="display:block;">
								<div id="lisHeight" class="p20">
									<form target="hidframe"  action="<?php echo M_URL($this->AppName.'/Index','batchNotice',$classroomid,GP('classroomid-'.$classroomid)) ?>" method="post" id="myform">
									...
									<table width="100%" cellpadding="0" cellspacing="0" border="0" class="ks-table">
										...
										<tr>
											...
											<td><?php echo Field($v['noticetitle'])?></td>
											<td><?php echo Field($v['content'])?></td>
											<td><?php echo Field(date("Y-m-d H:i",$v['adddate']))?></td>
											<td>
												<a href="javascript:void(0);" class="c-92 pr10 pl10 r-icon posRe"  onclick="Confirm('是否删除此公告？该操作不可逆!','<?php echo M_URL($this->AppName.'/Index','delNotice',$v['id'],GP('classroomid-'.$classroomid)) ?>')">
