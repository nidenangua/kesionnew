...
class CommonController extends Controller
{
   public  function _initialize()
   {
		...
		$this->AppName     = 'column';
		$this->appid       = null !==GF('appid') ? GF('appid'):'106';
        ...
        check_power($this->userinfo,'column','share');//检查管理员权限
		...
		$this->AppModel    = M('app_column');
		$this->columnid    = null !==GF('columnid') ? GF('columnid'):'0';
		if($this->columnid == 0){
			$this->columnid=ChkClng(KS_G('get.id'));
		}
    }
}