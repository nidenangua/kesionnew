...
<body class="ks-wrap">  
 ...
 <div class="ks-wbox bRadius10">
	...
	<!--标题-->
	    <a href="">社群管理</a><span><em>/</em>绑定课程</span>
	<!--标题-->
	</div>
	<div class="topCourseBox">
		...
		<div class="topCourseBox-img" onclick="addTypeChange('','930px','610px',{title:'选择图片',type:'top'});">
				<img onerror="this.src='<?php echo XImg($Info['image_path'])?>'" src="<?php if($Info['image_path']){echo Img($Info['image_path']);}else{echo nopic(12);}?>" class="defaultpic" width="100%" height="100%">
		</div>
		...
		<div class="topCourseCont">
			<div class="topCourseCont-left">
				...
				<div class="mt10">
					<div class="mb10">
							<span class="fsize14 c-92">成员数： <?php echo $Info['member_num']?>	</span><br>
							<span class="fsize14 c-92">动态数： <?php echo $Info['topic_num']?>	</span>
					</div>
                </div>
            ...
            <div class="topCourseBox-top">
                <ul class="clearfix">
                    <li><a href="<?php echo M_URL($this->AppName.'/Index','editTeam',$Info['id'])?>">编辑社群</a></li>丨
                    <li><a href="javascript:addTypeChange('<?php echo M_URL($this->AppName.'/Index','share',$Info['id'],GP(''));?>','700px','400px',{title:'推广'});">推广</a></li>
                    <!--<li>分享设置</li>-->
                </ul>
            </div>
...
<div class="ks-wbox mt20 bRadius10">
    ...
    <div class="ks-search-box fl">
        ...
        <select class="w150 inputText bRadius5 fl mr10" id="keytype" onchange="location.href='<?php echo M_URL($this->AppName.'/Index','serialization',$id); ?>?keytype-'+this.value;">
			...
			<?php if(ismoduleopen($this->wxid,6,2)){?><option value="2" <?php if($keytype==2){echo 'selected="selected"';} ?>>语音</option><?php }?>
			<?php if(ismoduleopen($this->wxid,30)){?><option value="5" <?php if($keytype==5){echo 'selected="selected"';} ?>>直播</option><?php }?>
		</select>
    ...
    <div class="fr">
        ...
        <li>
            ...
            <a class="ks-heade-button ks-head-primary" type="button"  onclick="addTypeChange('<?php echo M_URL($this->AppName.'/Index','addshop',$Info['id'],GP('appid-35'));  ?>','1000px','650px',{title:'添加商品',type:'top'});">
         ...
         <table class="ks-table">
            ...
            <tbody>
                <?php foreach($ary_link as $k => $v){?>
                    ...
                    <td> <?php if($v['courseType']==1){echo '点播';}elseif($v['courseType']==2){echo '语音';}elseif($v['courseType']==3){echo '音频';}elseif($v['courseType']==4){echo '图文';}elseif($v['courseType']==5){echo '直播';}?></td>
