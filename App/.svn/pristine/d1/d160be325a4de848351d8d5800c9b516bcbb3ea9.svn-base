			...
			<!--START HEAD-->
				<div class="clearfix">
					<div class="form-tip clearfix">
						...
						<span class="form-tip-word fl">班级学员</span>
						<span class="fr">
							<a href="javascript:void(0)" class="ks-bt bt-def mr10" onclick="addTypeChange('<?php echo M_URL($this->AppName.'/Index','addStudent',$classroomid,GP('')) ?>','850px','600px',{title:'选择学员',type:'top'})" id="addStudent">
						...
						<form target="hidframe"  class="fr">
							<div class="ks-top-search">
								...
								<input type="submit" class="sc-button"  onClick="searchByClass('<?php echo GP(''); ?>','<?php echo M_URL($this->AppName.'/Index','showStudent',$classroomid); ?>','<?php echo URL_MODEL;?>');" name="searchBtn" value="搜索" >
							</div>
						</form>
						...
						<div class="stuConBox">
							<div class="studentBox" style="display:block;">
								<div id="lisHeight" class="p20">
									<form target="hidframe"  action="<?php echo M_URL($this->AppName.'/Index','batchStudent',$classroomid,GP('classroomid-'.$classroomid)) ?>" method="post" id="myform">
									...
									<table width="100%" cellpadding="0" cellspacing="0" border="0" class="ks-table">
										...
										<tr>
											...
											<td>
												<a href="javascript:void(0);" class="c-92 pr10 pl10 r-icon posRe"  onclick="Confirm('是否删除此学员？该操作不可逆!','<?php echo M_URL($this->AppName.'/Index','delStudent',$v['id'],GP('classroomid-'.$classroomid)) ?>')">
