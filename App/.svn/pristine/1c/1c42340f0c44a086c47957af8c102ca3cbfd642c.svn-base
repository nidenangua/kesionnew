...
.socre-card .lstbtns button.col{color:#f28550!important;border:#f28550 1px solid!important;}
	...
	<div id="score" >
		...
		<div class="score-box">
			...
			<div class="score-show"><span id="scores">97</span>/<a id='sumscore'>100</a>分</div>
		</div>
		...
		<div class="classmin-topTab blue-box_shadow"> 
			<ul class="clearfix">
				<li class="active"><div>
					<p>总题数</p>
					<p><span id="sum">20</span > 道</p>
				</div></li>
				<li class="active"><div>
					<p>答对题数</p>
					<p><span id="good">18</span> 道</p>
				</div></li>
				<li class="active"><div>
					<p>正确率</p>
					<p><span id="pec">90%</span></p>
				</div></li>
			</ul>
		...
		<!--<div class="score-onten blue-box_shadow">
			<ul class="list">
				<li class="exam-title">英语加强班第一次作业</li>
				<li>班级/课程：班级</li>
				<li>所属班级/课程名称：英语加强班</li>
				<li>作业用时：1小时12分51秒</li>
			</ul>
		</div>-->
		<footer class="footer">
	...
	<div id="read" style="display: none;">
		<div class="myTask" style="display: block;">
...
<?php 
	...
	$sumscore = 0;
	$showdata1=array();
	foreach($memberanswer as $k=>$v){   
		$sumscore += ($v['score']*count($v['tks']));
		foreach($v['tks'] as $kt=>$vt){ 
			if(!empty($vt['answer'])){
				if($vt['tk']['tktype'] == 1 || $vt['tk']['tktype'] == 6){   //单选题 
				 	$a =range('A','Z') ;
				 	$option =explode('$$$',$vt['tk']['options']);
				 	$select=explode('|$|',$option[1]);
				 	$showdata2=array("tktype"=>$vt['tk']['tktype'],"title"=>htmlentities($option[0],ENT_QUOTES,"UTF-8"),"stuanswer"=>$vt['result']['answer1'],"answer"=>$vt['result']['answer'],"analysis"=>htmlentities($vt['result']['analysis'],ENT_QUOTES,"UTF-8"),"totalscore"=>$v['score'],"score"=>$vt['result']['score']);
					
				}elseif($vt['tk']['tktype'] == 2){//填空题
					$find = '{#answer}';
					$repl = '_______';
					$options = str_replace($find,$repl,$vt['tk']['options']);
					$showdata2=array("tktype"=>$vt['tk']['tktype'],"title"=>htmlentities($vt['tk']['options'],ENT_QUOTES,"UTF-8"),"stuanswer"=>$vt['result']['answer1'],"answer"=>$vt['result']['answer'],"analysis"=>htmlentities($vt['result']['analysis'],ENT_QUOTES,"UTF-8"),"totalscore"=>$v['score'],"score"=>$vt['result']['score']);	
					
				}elseif($vt['tk']['tktype']==3){ //判断题
				...
				}elseif($vt['tk']['tktype']==4){//问答题
					$stem4 = $vt['tk']['options'];
					$anser = explode('$$$', $vt['tk']['anser']);
					$anser4 = $anser[0];
					$showdata2=array("tktype"=>$vt['tk']['tktype'],"title"=>htmlentities($anser4,ENT_QUOTES,"UTF-8"),"stuanswer"=>$vt['result']['answer1'],"answer"=>$vt['result']['answer'],"analysis"=>htmlentities($vt['result']['analysis'],ENT_QUOTES,"UTF-8"),"totalscore"=>$v['score'],"score"=>$vt['result']['score']);	
					
				}elseif($vt['tk']['tktype']==5){//材料题
				...
				$sum++;
				array_push($showdata1,$showdata2);
			}
		}
	} 
	//;
	$array=array('score'=>$score,'good'=>$good,'sum'=>$sum,'sumscore'=>$sumscore,'pec'=>sprintf("%.1f",($good/$sum*100))."%");
?>
		<script src="<?php echo UOOT ?>Public/common/js/z.methodLibrary.js?version=1.0"></script>
		<script type="text/javascript">
			var trueid = '<?php echo $taskanswer['taskid']?>'; 
			'use strict';

var arraystr = JSON.parse('<?php echo json_encode($array); ?>');
...
var myTask = new Object({
	...
	showpec: function showpec() {
		$_id('scores').innerHTML = arraystr.score;
		$_id('good').innerHTML = arraystr.good;
		$_id('sum').innerHTML = arraystr.sum;
		$_id('pec').innerHTML = arraystr.pec;
		$_id('sumscore').innerHTML = arraystr.sumscore;
	},
	...
	showStim: function showStim() {
		$_id('score').style.display = 'none';
		$_id('read').style.display = 'block';
	},
	upload: function upload() {
		$_id('fileid').click();
	},
	uploadinof: function uploadinof(e) {
		var exe = e.value.replace(/.+\./, ""); //正则表达式获取后缀
		var name = e.value.replace(/^.+?\\([^\\]+?)(\.[^\.\\]*?)?$/gi, "$1"); //name
		$_id('filename').innerHTML = name + '.' + exe;
	},
	maskopen: function maskopen() {
	...
	gotos: function gotos(id) {
		myTask.page = id - 1;
		myTask.showTim();
	},
	nextTim: function nextTim() {
	...
	//检查当前题目是否标记
	check_now_put: function check_now_put() {
		$_id('put').setClass('active', 'remove');
		var pust = myTask.page + 1;
		$_class('lstbtn').map(function ($v, $i) {
			$v.findTag('button').map(function ($elem, $index) {
	...
	//检查当前题目是否收藏
	check_collect_put: function check_collect_put() {
		$_id('collect').setClass('active', 'remove');
		var pust = myTask.page + 1;
		$_class('lstbtn').map(function ($v, $i) {
			$v.findTag('button').map(function ($elem, $index) {
	...
	//检查当前题目是否收藏
	check_collect_card: function check_collect_card() {
		$_class('lstbtn').map(function ($v, $i) {
			$v.findTag('button').map(function ($elem, $index) {
				var gettip = $elem.getAttribute('collect');
				var gettip = $elem.getAttribute('collect');
	...
	postar: function postar(id, type) {
		if (type == 'add') {
			var ajax = {
	...
	//点亮
	linght: function linght(num, flag) {
		var less = 0;
	...
	list: function list(id) {
		...
		$_post(ajax.url, ajax.data, function (res) {
			$_id('scores').innerHTML = res.taskDetaile.answer['score'];
			//						$_id('good').innerHTML = arraystr.good;
			console.log(res.taskDetaile.answer['score']);
		}, 'json');
	},
	//反向解析
	repush: function repush() {
	...
	//创建答题卡
	creatTim: function creatTim(data, len, timnum) {
		var pushdata = '';
	...
	//创建试题
	creatTimu: {
		choose: function choose(data, len, timnum) {
			var tk = data.tk;
		...
		//创建材料题
		creatcai: function creatcai() {},
		//选择题
		timu1: function timu1(data, len, timnum) {
		...
		//填空题
		timu2: function timu2(data, len, timnum) {
		...
		//判断题
		timu3: function timu3(data, len, timnum) {
			var pushdata = '<li tktype="' + data.tktype + '"><div class="list-title blue-box_shadow clearfix"><span>' + timnum + '、</span>' + data.option + '</div>';
			pushdata += '<ul class="myTask-option">';
			pushdata += '	<li>A、错误</li>';
			pushdata += '	<li>B、正确</li>';
		...
		//问答题
		timu4: function timu4(data, len, timnum) {
			var pushdata = '<li tktype="' + data.tktype + '"><div class="list-title blue-box_shadow clearfix"><span>' + timnum + '、</span>' + data.option + '</div>';
		...
		//材料题
		timu5: function timu5(data, len, timnum) {
		...
		//多选题
		timu6: function timu6(data, len, timnum) {
			var pushdata = '<li tktype="' + data.tktype + '"><div class="list-title blue-box_shadow clearfix"><span>' + timnum + '、</span>' + data.option + '</div>';
			pushdata += '<ul class="myTask-option">';
			data.select.map(function ($v, $i) {
		...
		//文件题
		timu7: function timu7(data, len, timnum) {
			var pushdata = '<li tktype="' + data.tktype + '"><div class="list-title blue-box_shadow clearfix"><span>' + timnum + '、</span>' + data.option + '</div>';
