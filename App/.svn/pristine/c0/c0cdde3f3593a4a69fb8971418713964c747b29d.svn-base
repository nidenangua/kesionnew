...
<body class="gray-bg">
	...
	<div id="ksBoxHead" >
		...
		<div class="Header-tool clearfix">
			...
			<!--图像-->
			<!--标题-->
			<!--<p><a   href="<?php echo M_URL($this->AppName.'/Index','index','',GP('')); ?>" target="main">直播管理</a></p>-->
			<p>
				<a href="<?php echo M_URL($this->AppName.'/Index','index','',GP('')); ?>">
					<i class="iconfont icon-jiantou-copy"></i>
					直播管理</a><span>/</span>并发管理
			...
			<div class="fr" style="    width: 418px;">
						<form action="<?php echo  M_URL($this->AppName.'/Index','superveneExcel','',GP('wxid-'.$wxid.',p-'.$page->now_page.',stype-'.$type.',adddate-'.$adddate.',numb-'.$numb)); ?>" method="post">
			...
			<div class="fl ks-top-search">
				...
				<form target="hidframe" class="fr">
					<input type="text" class="sc-text" name="searchText" id="keyword" value="<?php if(isset($keyword)) echo $keyword; ?>">
	           		<input type="button" class="sc-button fl" onClick="searchByClass('<?php echo GP(''); ?>','<?php echo M_URL($this->AppName.'/Index','showSupervene'); ?>','<?php echo URL_MODEL;?>')" name="searchBtn" value="搜索">
				</form
			</div>
		...
		<div >
			<div style="padding: 8px 0px;">
					...
					<ul class="H5-gameOpt clearfix">
						...
						<li>
							<div class="shaixuan clearfix">
								<?php if($adddate == 1){?>
									<a class="desc" href="<?php echo M_URL($this->AppName.'/Index','showSupervene','',GP("adddate-2")); ?>"><i class="desc-i"></i>购买时间</a>		
								<?php }else{ ?>
									<a class="desc" href="<?php echo M_URL($this->AppName.'/Index','showSupervene','',GP("adddate-1")); ?>"><i class="asc-i"></i>购买时间</a>
								<?php }?>
							</div>
						...
						<li >
							<div class="shaixuan clearfix">
								...
								<?php if($numb == 1){?>
									<a class="desc" href="<?php echo M_URL($this->AppName.'/Index','showSupervene','',GP("numb-2")); ?>"><i class="desc-i"></i>购买数量</a>		
								<?php }else{ ?>
									<a class="desc" href="<?php echo M_URL($this->AppName.'/Index','showSupervene','',GP("numb-1")); ?>"><i class="asc-i"></i>购买数量</a>
								<?php }?>
							</div>
						</li>
						<li></li>
						<li class="fr gameScreen" style="margin-right: 20px;">
							...
							<ul class="H5-gameScreen">
								...
								<li>
									<span>时限类型：</span>
									<div>
										<a href="<?php echo M_URL($this->AppName.'/Index','showSupervene','',GP('p-'.$page->now_page.',stype-3'));?>" class="<?php if($type ==3){ echo 'curr';}?>">不限</a>
										<a href="<?php echo M_URL($this->AppName.'/Index','showSupervene','',GP('p-'.$page->now_page.',stype-1'));?>" class="<?php if($type ==1){ echo 'curr';}?>">过期</a>
										<a href="<?php echo M_URL($this->AppName.'/Index','showSupervene','',GP('p-'.$page->now_page.',stype-2'));?>" class="<?php if($type ==2){ echo 'curr';}?>">未过期</a>
									</div>
								</li>
							</ul>
				...
				<div class="H5-gameListConts school-online">
					<form action="<?php echo M_URL($this->AppName.'/Index','','',GP('p-'.$page->now_page)); ?>"  method="post">
							...
							<table class="ks-table" id="tables">
								...
								<tbody>
									...
									<tr>
										...
										<td>
											<?php echo Field($v['market_price']).'/人/月';?>
										</td>
										<td>
											<?php echo Field($v['price']).'/人/月' ;?>
										</td>
										<td>
											<?php echo Field($v['numb']);?>
										</td>
										<td>
											<?php echo Field($v['sumprice']) ;?>
										</td>
										<td>
											<?php echo date('Y/m/d',$v['starttime']).'-'.date('Y/m/d',$v['endtime']) ;?>
										</td>
										<td>
										...
										<td>
											<!--<div class="operate">
												<a href="javascript:void(0);" onclick="Confirm('是否删除该条记录？该操作不可逆!','<?php echo M_URL($this->AppName.'/Index','singledelete',$v['id'],GP('p-'.$page->now_page.',type-1')); ?>')">
