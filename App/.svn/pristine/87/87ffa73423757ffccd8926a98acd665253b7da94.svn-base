...
<body class="ks-wrap">
	<div class="ks-wbox bRadius10">
	   ...
	   <div class="ks-head-nav">
		...
		<form target="hidframe" method="post" id="myform" action="<?php echo M_URL($this->AppName.'/Index','doAddTuwen','',GP("")); ?>" class="form-horizontal" enctype="multipart/form-data">
		<div class="moBan-left">
			
			<ul class="moBan-ul">
			<input type="hidden" name="sunList" id="sunList" value="1" />
			<input type="hidden" name="sunLength" id="sunLength" value="1" />
			<input type="hidden" name="countList" id="countList"   value="" />
			<li id="model-1" class="xz"  onclick="meau(this,'+num+')">
				<div class='moBan-imgBox moNum' id="moBan-imgBox">
				<div class="deault-box">
				<img class="showImg" src="/Public/uploads/common/timg.jpg" alt="图片"/>
				<p class="moBan-boxMask" id="title1" style="text-align: center;"><span class="titleMark"  style="text-align: center;overflow : hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical;"></span></p>
				</div>
				</div>
				<input type="hidden"  class="form-textbox keytype" name="keytype1" value="0">
				<input type="hidden"  class="form-textbox title yanzheng" name="title1" value=""> 
				<input type="hidden"  class="form-textbox author  yanzheng" name="author1" value="" >
				<input type="hidden"  class="form-textbox text yanzheng" name="text1" value="" >
				<input type="hidden"  class="form-textbox abstract yanzheng" id="abstract1" name="abstract1" value="" >
				<input type="hidden"  class="form-textbox linkurl yanzheng" name="linkurl1" value="">		
				<input type="hidden"  class="form-textbox pic yanzheng" name="pic1" value="/Public/uploads/common/timg.jpg">		
				<input type="hidden"  class="form-textbox parentID" name="parentID" value="0">
		...
		<div class=" wrapper-content animated fadeInRight">
			<div class="tabs-containers">
				<div class="form-row clearfix" id="keytype">
					<label><input type="radio" class="from" name="from" id="" value="zidingyi" checked="checked"/>自定义</label>
					...
					<div  class="moBan-tab"  style="display: block;" id="zidingyi">
						<div class="panel-bodys">
							...
							<textarea rows="4"  class="form-textarea"  oninput="dataInput()" name="abstract"><?php if(!empty($tuwen)){echo $tuwen['abstract'];}?></textarea>
							</div>
							</div>
							...
							<divclass="clearfix" >
							<input type="text"  class="form-textbox" oninput="dataInput()" name="linkurl" value="<?php if(!empty($tuwen)){echo $tuwen['linkurl'];} ?>">
							</div>
							</div>
							...
							<div class="js_uploadBox">
		              			<div class="js_showBox bRadiusn"><img id="defalutimage" class="js_logoBox block" src="<?php if(!empty($course["defaultpic"])){echo Img($course["defaultpic"]);}else{ echo nopic(1);} ?>"  ></div>
		...
		<!--footer-->
		<div class="ks-bom clearfix" style="text-align: center;">
            	<button type="button"  onClick="do_reg()" class="ks-bt bt-pue fn mr20">确认</button>
           		<button type="button" onclick="javascript:history.go(-1);" class="ks-bt bt-def fn">取消</button>
		</div>
		<!--/footer-->
