        ...
        <div class="fl">
            ...
            <form target="hidframe">
                ...
                <select class="w150 inputText bRadius5 fl" id="keytype">
					<option value="2" <?php if($keytype==2){echo 'selected="selected"';} ?>>用户账号</option>
					<option value="1" <?php if($keytype==1){echo 'selected="selected"';} ?>>用户昵称</option>
			  	</select>
                <div class="fl ks-top-search">
                    <input type="text" class="sc-text " name="searchText" id="keyword" value="<?php if(isset($keyword)) echo $keyword; ?>" >
                    <button type="submit" class="ks-head-search" onClick="searchByClass('<?php echo GP(''); ?>','<?php echo M_URL($this->AppName.'/Index','addgroupuser',$id); ?>','<?php echo URL_MODEL;?>')"><i class="iconfont icon-sousuo"></i></button>
                </div>
            </form>
        </div>          
    </div>      
    ...
    <table class="ks-table" style="    margin-top: 32px;">
        ...
        <?php foreach ($values as $k => $v) { ?>
            <tr>
                ...
                <td style="text-align: left;"><?php echo $v['nickname']; ?></td>
...
<div class="clearfix ks-popup-footer">
    <div class="fl mt20">
        ...
        <form target="hidframe"  id="myform" class="form-horizontal" action="<?php echo M_URL($this->AppName.'/Index','doGroupAddUser',$id,GP(''));  ?>" method="post" enctype="multipart/form-data">
