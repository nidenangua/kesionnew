...
.mTask .screelst-lstbox>ul{position: absolute;bottom:2.7rem;overflow: auto;top: 2.5rem;width: 100%;height: 480px;} 
...
.tishi{
    ...
    border: #f28550 1px solid!important;
}
#tishi-box div{
	font-size: .7rem;
...
.stamp{
	    border: #37A7ED 1px solid!important;
}
.collect-box{
	border: #f28550 1px solid!important;
}
			...
			<section class="mTask" id="filler"> 
				...
				<div class="mTask-sen" id="mTasksen" style="margin-right: -80r%;">
					...
					<div class="screelst-lstbox">
						...
						<div id="tishi-box">
							<ul>
								<li><div class="write-box"></div>已做</li>
								<li><div ></div>未做</li>
							    <li><div class="stamp"></div>标记</li>
								<li><div class="collect-box"></div>收藏</li>
							</ul>
						</div>
		...
		<script type="text/javascript">
			...
			var myTask = new Object({
			        ...
			        ajax.onreadystatechange = function () {
			            if (ajax.readyState==4&&ajax.status==200) {
			                if(ajax.responseText.submitjob2.result=="success"){
			                	alert('提交成功！');
			                	window.location.href='https://<?php echo APP_DOMAIN;?>/h5market/myWork';
				...
				postform:function(){
					...
					var index = 0;
					var tskidstr = 0;
					$_id('dtimcard').querySelectorAll('li').map(function(item){//拼接做过的题目
						item.querySelector('.lstbtn').querySelectorAll('button').map(function(item2){
							if(item2.className=='active'){
								tskidstr+=','+item2.getAttribute('taskid');
//								questionArr.push(item2.getAttribute('taskid'))
							}else{
								tskidstr+=',0';
							}
						})
...
//					var taskary = $_class('lstbtn').attr('taskid');
//					console.log(taskary)
					var _equ =''
					for(var e=0;e<Equal.length;e++){
						_equ+=Equal.charCodeAt(e)
					}
					// console.log(_equ)
					var ajax={
						url:'&taskid='+myTask.tkids+'&answer='+Equal+'&tkids='+tskidstr+'&time='+$_id('tim_time').innerHTML,
					}
					// return;
				...
				ajaxpost:function(data){
					var model = '';
					modeltype == 1? model = 'Class' : model = 'Microportal';
					modeltype == 1? model2 = 'myWork' : model2 = 'myPaper';
					 var ajax = new XMLHttpRequest();
			        // 使用post请求
			        ajax.open('post','/Json/index.php?act='+model+'.submitjob1');
			        ...
			        ajax.onreadystatechange = function () {
			            if (ajax.readyState==4&&ajax.status==200) {
			                ...
			                console.log(res);
			                if(res.submitjob1=='error'){
                               alert('提交失败');
			                }else if(res.submitjob1.status=='ok'){
			                	if(res.submitjob1.status=='ok'){
			                		if(res.submitjob1.scoreout==1){
                                        var smtip = confirm('您的成绩现在可去查看，是否前去');
										if(smtip==true){
											 window.location.href=res.submitjob1.res.url;
										}else{
										    window.location.href='https://<?php echo APP_DOMAIN;?>/h5market/'+model2;
										}
			                		}else{
                                        alert('提交成功！耐心等待老师批阅');
			                           window.location.href='https://<?php echo APP_DOMAIN;?>/h5market/'+model2;	 
			                ...
			                }else if(res.submitjob1.status=='no'){
                                        alert(res.submitjob1.msg);	 
			                		
			                }else if(res.submitjob1=='success'){
			                	 alert('提交成功！');
			                     window.location.href='https://<?php echo APP_DOMAIN;?>/h5market/myWork';
			                }
					...
					$_class('lstbtn').map(function($v,$i){
						$v.findTag('button').map(function($elem,$index){
							...
							if($elem.innerHTML*1==pust){
								...
								console.log(gettip)
								if(gettip=='false' || gettip=="undefined"){
									$_id('collect').setClass('active','add');
									$elem.setAttribute('collect','true');
									var tac = $_id('Timulist').findClass('active')[0].getAttribute('subject');
									myTask.postar(tac,'add');
								}else{
									$_id('collect').setClass('active','remove');
									$elem.setAttribute('collect','false');
									var tac = $_id('Timulist').findClass('active')[0].getAttribute('subject');
									myTask.postar(tac,'del');
				...
				list:function(id){
					...
					modeltype == 1? model = 'Class' : model = 'Microportal';
					var ajax = {
						url:'/Json/index.php?act='+model+'.taskDetaile&id='+id,
						data:''
					}
					$_post(ajax.url,ajax.data,function(res){
						if(res.taskDetaile.error){
                             alert('试卷出错');
			                 window.location.href='https://<?php echo M_URL('h5market','myPaper')?>';
			                 return;
						}
						if(res.taskDetaile.task){
				...
				creatTim:function(data,len,timnum){
					...
					len.map(function(val,i){
						pushdata 	+='<button tip="'+val.tip+'" taskid="'+val.tkid+'" collect="'+val.collect+'" onclick="myTask.gotos('+(i+timnum)+')">'+(i+timnum)+'</button>';
					})
					pushdata 	+='</li>';
					return pushdata;
				},
				...
				creatTimu:{
					...
					timu1:function(data,len,timnum){
						var pushdata = '<li tktype="'+data.tktype+'" subject="'+data.tkid+'"><div class="list-title blue-box_shadow clearfix"><span>'+(timnum)+'、</span>'+data.option+'</div>';
					...
					timu2:function(data,len,timnum){
						...
						var pushdata = '<li tktype="'+data.tktype+'" subject="'+data.tkid+'"><div class="list-title blue-box_shadow clearfix">';
					...
					timu5:function(data,len,timnum){
						var pushdata = '<li tktype="'+data.tktype+'" subject="'+data.tkid+'"><div class="blue-box_shadow timu5"><div class="list-title nobt clearfix"><span>'+(timnum)+'、</span>'+data.option+'</div>';
