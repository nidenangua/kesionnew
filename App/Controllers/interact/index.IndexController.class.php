...
class IndexController extends CommonController
{
	 ...
	 public function InteractAjaxAction(){
		...
		foreach($interact as $k=>$v){$headurl = ''; 
			if($v['membername']==''){$interact[$k]['membername']  = '游客';}
			elseif($v['anonymous']==1){$interact[$k]['membername']  = '匿名';}
			else{$headurl = $this->upload->getHead($v['memberid']);}
			$interact[$k]['headurl']  = $headurl ? $headurl :'';
			$interact[$k]['interact'] = ShowEmot($v['interact'],$this->getInstall());
			$interact[$k]['adddate']  = date('Y-m-d H:i',$v['adddate']);
		}
		$result['values']  =  $interact; 
		$result['page']    =  $page; 
	 ...
	 public function doaddInteractAjaxAction(){
		...
		if($appid==6){
			...
			$app_news          = M('app_news');
			$Datatwo['cmtnum'] = $this->AppModel->getOne('count(*)','where wxid='.$GLOBALS['_DoM']['wxid'].' and infoid='.$infoid.' and appid='.$appid);
			$Datatwo['cmtnum'] = $Datatwo['cmtnum']+1;
			$app_news          ->update($Datatwo,'wxid='.$GLOBALS['_DoM']['wxid'].' and id='.$infoid);
		}
		$Data['appid']      = $appid;
		$Data['infoid']     = $infoid;
...
//elseif(intval($appid-100)>=0){//$Data['membername'] = ChkSQL(KS_G("post.username"));//}
		$Data['adddate']    = time();
	 ...
	 public function doaddImageAjaxAction(){
		 ...
		 if(!$info) {$this->error($upload->getError());}//上传错误提示错误信息
		 else{// 分离缩略图和轮播图
		    header("Content-type: application/json");
	 ...
	 public function doaddReplyAjaxAction(){
		...
		$Datas['replynum'] = 'replynum+1';
		$this->AppModel->update($Datas,'wxid='.$GLOBALS['_DoM']['wxid'].' and interactid='.$interactid,1);
		header("Content-type: application/json");
