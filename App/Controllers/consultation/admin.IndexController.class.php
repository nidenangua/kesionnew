...
//		exit;
		...
		include  CURR_VIEW_PATH . "coupon_manage.php";
	}
	//显示添加页面
	public function couponaddAction(){
		...
		$now_page =  ChkClng(KS_S('p',1));		
		include  CURR_VIEW_PATH . "coupon.php";
	}
	//显示生成优惠券页面
	public function buildAction(){
		$couId = ChkSQL(KS_G('request.id'));
		$now_page =  ChkClng(KS_S('p',1));
		include  CURR_VIEW_PATH . "coupon_build.php";
	}	
	...
	public function addcouponAction(){
		...
		if($coupontype==1){
			...
			$table->doadd($data,'id');
			KS_INFO("添加成功",0,M_URL($this->AppName.'/Index','couponadd','',GP(''))); 
	...
	public function buildcouAction(){
        ...
        if(intval($buildnum)>1){
	        ...
	        if(intval($detailvou['maxnum'])<intval($buildnum)){
	        	KS_INFO("你输入的数值超过了最大值,请重新去生成",0,M_URL($this->AppName.'/Index','index','',GP('')),'',1);
	        }
		...
		include  CURR_VIEW_PATH . "jdt.php";
	    for ($i = 0; $i <=$buildnum ; $i ++) { 
            ...
            if($jg==100){
            	KS_INFO("优惠券生成完毕",0,M_URL($this->AppName.'/Index','index','',GP('')),'',1);
            }
		}
	}
	/**
	...
	public function editcouponAction(){	
		...
		if($coupontype==1){
			...
			KS_INFO("修改成功",0,M_URL($this->AppName.'/Index','index','',GP(''))); 
	}
	/**
	 * 单个删除
	 */
...
//exit;
		...
		include  CURR_VIEW_PATH . "coupon_grant.php";	
...
//		var_dump($couponAry);
	...
	public function grantAction(){
        ...
        if($type==1){
        	if(count($wxid)>1){
        		$url = M_URL($this->AppName.'/Index','showgrant','',GP('type-'.$type.',maxid-'.$addid.',j-'.$coupon));
        		KS_INFO("只能选择一个人",0,$url,'',1);	
        	}else{
				$Data['wxid'] = $wxid[0];
...
//	}
	...
	public function showDetailAction(){
		...
		include  CURR_VIEW_PATH . "coupon_detail.php";
	}
