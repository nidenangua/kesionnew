...
<form target="hidframe" id="myform" class="form-horizontal" action="<?php if(empty($value)){echo M_URL('Label','LabeldoAdd','',GP('labeltype-'.$labeltype));}else{echo M_URL('Label','LabeldoEdit',$labelid,GP('p-'.$now_page));} ?>" method="post" enctype="multipart/form-data">
        ...
        <div class="line line-dashed b-b line-lg pull-in"></div>
        <div class="form-group">
            <label class="col-sm-2 control-label">选择课程类型：</label>
            <div class="col-sm-2 w200">
               <select class="form-control" id="courseType" <?php if(!empty($value)){echo 'disabled';} ?> >
                   <option value="0">-选择课程类型-</option>
                   <option value="1">-直播应用-</option>
                   <option value="2">-点播应用-</option>
                   <option value="3">-面授应用-</option>
               </select>
            </div>
            <label class="control-label fl">指定具体应用，获得自定义字段，选中后不要修改</label>
            <?php if(!empty($value)&&!empty($courseType)){?>
			<script>
                var v = '<?php echo $courseType; ?>';
				for(var i=0;i<$("#courseType")[0].options.length;i++)
				{
					if(v==$("#courseType")[0].options[i].value)
					$("#courseType")[0].options[i].selected=true;
				}  
            </script>
            <?php }?>
            <!--
            onChange="getfield(this,'<?php echo UOOT.'Plus/Kesion.ajax.php?act=getfield'; ?>');"
            <script>
			function getfield(obj,url)
			{
				...
				if(ctype!='0')
		        {
					$('#field').html('<li onclick="InsertLabel(\'courseid\',\'-1\')">课程ID</li><li onclick="InsertLabel(\'userid\',\'-1\')">用户ID</li><li onclick="InsertLabel(\'categoryid\',\'14\')">分类</li><li onclick="InsertLabel(\'title\',\'1\')">标题</li><li onclick="InsertLabel(\'defaultpic\',\'1\')">默认封面</li><li onclick="InsertLabel(\'enablechapter\',\'1\')">是否开启章节</li><li onclick="InsertLabel(\'chargetype\',\'1\')">章节类型</li><li onclick="InsertLabel(\'adddate\',\'15\')">添加时间</li><li onclick="InsertLabel(\'teacherid\',\'1\')">老师ID</li><li onclick="InsertLabel(\'intro\',\'1\')">课程介绍</li><li onclick="InsertLabel(\'price_market\',\'1\')">课程参考价</li><li onclick="InsertLabel(\'discount\',\'1\')">折扣</li><li onclick="InsertLabel(\'price\',\'1\')">实际销售价</li><li onclick="InsertLabel(\'validedays\',\'1\')">有效天数</li><li onclick="InsertLabel(\'recommend\',\'6\')">是否推荐</li><li onclick="InsertLabel(\'slide\',\'6\')">是否幻灯</li><li onclick="InsertLabel(\'hits\',\'1\')">课程人气</li><li onclick="InsertLabel(\'soldnum\',\'1\')">课程销售量</li><li onclick="InsertLabel(\'status\',\'5\')">审核状态</li><li onclick="InsertLabel(\'chaptertable\',\'6\')">章节分表名</li><li onclick="InsertLabel(\'content\',\'6\')">课程详细介绍</li><li onclick="InsertLabel(\'announce\',\'6\')">课程公告</li>');
				}else{
					$('#field').html('');
			    }
			}
