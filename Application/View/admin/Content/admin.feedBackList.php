			...
			<div id="manage_top" class="menu_top_fixed  p15 height73">
				<div class="ibox-title">
					<div class="clearfix" style="float: left;">
						<h3 class="fl" style="float: left;">&nbsp;&nbsp;意见反馈管理</h3>
					</div>
					<div class="clearfix" style="float: left;margin-left: 56px;">
						<span class="btn btn-default btn-rounded "  style="cursor:pointer" onclick="addTypeChange('<?php echo M_URL('Content','sendMessage','',GP('p-'.$now_page.',backtype-2,allwx-2'));?>','940px','660px',{title:'群发to网校',type:'top'})">群发to网校</span>
					</div>
					<div class="fr">
						<div class="fl ks-top-search">
							...
							<form target="hidframe" class="fr">
								<input type="text" class="sc-text" name="searchText" id="keyword" value="<?php if($keyword){echo $keyword;}?>">
				           		<input type="button" class="sc-button fl" onClick="searchByClass('<?php echo GP(''); ?>','<?php echo M_URL('Content','feedBackList'); ?>','<?php echo URL_MODEL;?>')"   name="searchBtn" value="搜索">
							</form>
						</div>
					</div>
				</div>
...
<!-- 				<div class="H5-gameListScreen">
				...
				<div class="H5-gameListConts school-online">
					<form action="<?php echo M_URL('Content','deleteFeedBack','',GP('')) ?>"  method="post">
							...
							<table class="ks-table" id="tables">
								<thead>
									<tr style="background: #EEEEEE;">
										...
										<th style="width: 8%!important;">反馈用户</th>
										<th>网校名称</th>
										<th style="width: 30%!important;">反馈标题</th>
										<th style="width: 30%!important;">是否答复</th>
										<th class="shaixuan">
											<?php if($adddate == 2){?>
												<a class="desc" href="<?php echo M_URL('Setting','opinionlist','',GP("adddate-1")); ?>" style="float: none;"><i class="desc-i" style="top: -1px;"></i>反馈时间</a>		
											<?php }else{ ?>
												<a class="desc" href="<?php echo M_URL('Setting','opinionlist','',GP("adddate-2")); ?>" style="float: none;"><i class="asc-i" style="top: -1px;"></i>反馈时间</a>		
											<?php }?>
										</th>
										<th>操作</th>	
									</tr>
								...
								<tbody>
									...
									<tr>
										...
										<td><?php echo $v['isreply']==1?'已答复':'-'; ?></td>
										<td><?php echo date('Y-m-d H:i:s',$v['adddate']); ?></td>
										<td>
											<a class="mr20" style="color: #476DBE;"  onclick="addTypeChange('<?php echo M_URL('Content','feedBackShow',$v['id'],GP('')) ?>','960px','620px',{title:'查看回复',type:'top'})">查看回复</a>
											<a class="mr10" style="color: #476DBE;" onclick="Confirm('是否删除该条内容','<?php echo M_URL('Content','deleteFeedBack',$v['id'],GP('p-'.$page->now_page)); ?>')">删除</a>
										</td>
									</tr>
									<?php }?>
								</tbody>
