...
<div class="panel-heading">音乐管理-----/<?php echo $tempdir;?>----------<?php echo $name;?></div>
<table class="table " >
    <thead>
    <tr>
        <th>音乐目录</th><th>类型</th><th>大小</th><th>修改时间</th><th>操作</th>
    </tr>
    </thead>
    <tbody>
    <?php if(!empty($first)){?>
    <tr onClick="skip('<?php echo M_URL('Index','musiclist','',GP('name-'.$name.',type-'.$type,true));?>')">
        <td>(..顶层目录)</td><td></td><td></td><td></td><td></td>
    </tr>	
    <tr onClick="skip('<?php echo M_URL('Index','musiclist','',GP('tempdir-'.$tempdir.',back-1,name-'.$name.',type-'.$type,true));?>')">
        <td>(.上层目录)</td><td></td><td></td><td></td><td></td>
    </tr>
    <?php }?>
    <?php foreach($dirname as $k=>$v){?>
    <tr onClick="skip('<?php echo M_URL('Index','musiclist','',GP('tempdir-'.$tempdir.',nowdir-'.urlencode($v).',name-'.$name.',type-'.$type,true));?>')">
    ...
    <tr class="card-li">
        <td><audio  class="card-img" src="<?php echo '/'.$tempdir.'/'.iconv('gbk','utf-8',$v); ?>" style="width:100px;" controls="controls"/>音乐</audio></audio></td>
        <td>音乐</td>
        <td>3K</td>
        <td>2017/1/9 15:26:14</td>
...
function uploadView2(x,y,other){
	...
	uploader.on('error', function (code) {
		...
		switch (code) {
			...
			case 'Q_TYPE_DENIED':err += '无效图片类型，请上传正确的音乐格式！';break;
...
<script>
uploadView2(180,120,{picker:'#picker',filelist:'#filelist',url:'/Plus/Uploader.php?act=adminmusic'});
...
//		$(parent.document).find(".<?php echo $name;?>").attr('src',src);
		...
		$(parent.document).find('#J_iframe').contents().find('input[name="<?php echo $name;?>"]')[0].value = src;
//	    $(parent.document).find('#J_iframe').contents().find(".<?php echo $name;?>").attr('src',src);
	    top.closePopup(window.name);
	}else if(type=='other'){
//		top.imgUploadBack(src);
	    top.closePopup(window.name);
	}
})
</script>
