...
<table class="table " >
    ...
    <tbody>
    <?php if(!empty($first)){?>
    <tr onClick="skip('<?php echo M_URL('Index','imageslist','',GP('name-'.$name.',type-'.$type,true));?>')">
        <td>(..顶层目录)</td><td></td><td></td><td></td><td></td>
    </tr>	
    <tr onClick="skip('<?php echo M_URL('Index','imageslist','',GP('tempdir-'.$tempdir.',back-1,name-'.$name.',type-'.$type,true));?>')">
        <td>(.上层目录)</td><td></td><td></td><td></td><td></td>
    </tr>
    <?php }?>
    <?php foreach($dirname as $k=>$v){?>
    <tr onClick="skip('<?php echo M_URL('Index','imageslist','',GP('tempdir-'.$tempdir.',nowdir-'.urlencode($v).',name-'.$name.',type-'.$type,true));?>')">
    ...
    <tr class="card-li">
        <td><img class="card-img" src="<?php echo '/'.$tempdir.'/'.iconv('gbk','utf-8',$v); ?>" style="width:100px;height: 100px ;" /></td>
        <td>图片</td>
        <td>3K</td>
        <td>2017/1/9 15:26:14</td>
...
<script>
uploadView2(180,120,{picker:'#picker',filelist:'#filelist',url:'/Plus/Uploader.php?act=adminimages'});
...
$(".card-li").click(function(){
	...
	if(type=='parent'){
       ...
       var data='<img class="card-img" src="'+src+'"/>';
        if (top.frames['J_iframe'].contentWindow.UE!=undefined){
         top.frames['J_iframe'].contentWindow.UE.getEditor('content').execCommand('insertHtml',data);
    }else{
