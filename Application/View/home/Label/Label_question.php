...
<form target="hidframe" id="myform" class="form-horizontal" action="<?php if(empty($value)){echo M_URL('Label','LabeldoAdd','',GP('labeltype-'.$labeltype));}else{echo M_URL('Label','LabeldoEdit',$labelid,GP('p-'.$now_page));} ?>" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="hidden" value="12" id="appid">
            </div>
        </div>
        <div class="line line-dashed b-b line-lg pull-in"></div>
        <div class="form-group">
            <div class="col-sm-2 control-label">缓存时间：</div>
            ...
            <div class="control-label fl"> 分钟 设置为“0”不启用缓存，设置为“-1”永久缓存。</div>
            <div class="clearfix">
            对查询出来的数据没有经常更新的标签，建议设置一个缓存时间，减少数据库查询，整个系统的性能将得到很好的提升。
            </div>
        </div>
        <div class="line line-dashed b-b line-lg pull-in"></div>
        <div class="form-group">
        ...
        <div id="label-content" style="display: none;">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-8 taps-li">
                  <ul id="field"><li onclick="InsertLabel('id','-1')">问题ID</li><li onclick="InsertLabel('describe','1')">问题描述</li><li onclick="InsertLabel('problem','1')">问题</li></ul>
                </div>
            </div>
            <div class="line line-dashed b-b line-lg pull-in"></div>
            <div class="form-group">
    ...
    [/Loop]';}else{echo EncodeHtml($labelcontent);}} ?></textarea>
            ...
            <div class="form-group">
                <div class="control-label col-sm-2">简要描述</div>
                <div class="col-sm-4">
...
<script>
var getFormatUrl = '/Public/drag/label/ask.php'
if(location.href.indexOf('form-m')!=-1){
	getFormatUrl = '/Public/dragphone/label/ask.php'
}

format.methods.getFormatList({
    url:getFormatUrl,
    type:'ask'
});
