...
<style>
	...
	.Aconts-ul li{float: left; width: 20%;height: auto;}
...
<body>
	...
	<div class="Alert-box" style="width: 100%;">
					...
					<div class="Alert-conts clearfix">
						<ul class="Aconts-ul">
							...
							<li class="card-li"><div class="Aconts-box">								
								...
								<input type="checkbox" value="<?php echo $v['filePath'] ?>" class="file"/>
							</div>
							</li>	
							<?php }?>
		...
		<div style="display: none;" id="temp">
			<form action="<?php echo M_URL('Index','TempaddFile',$type1);?>"  method = "post" style="border: #000000 1px solid;background: white; border-radius: 5px; position: absolute;z-index: 10001;">
				<div  class="p20">
...
$('#enter').click(function(){
	$('.Aconts-ul li').each(function(){
		if($(this).find(".ctrShaw").css('display')=='block'){
			var a = $(this).parents().find("input").val();
			
			 var filename = $(this).find('.Alert-mark').text(); 
			data='';
			 if(type == 3){
	        var data='<embed height="100" width="100" src="'+a+'" /><br />';
	        }		
	    
		    $(parent.document).find('#main').contents().find('input[name="<?php echo $name;?>"]')[0].value = a;
