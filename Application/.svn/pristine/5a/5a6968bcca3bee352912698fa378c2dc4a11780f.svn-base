...
/*------------------------------------app应用分类表 查增改删-----------------------------------------------*/
class AppCategoryController extends CommonController
{	 
	 public function indexAction(){
    	$CategoryModel = new CategoryModel('common_app_categoryid');
		...
		include  CURR_VIEW_PATH . "AppCategory/appcategory_list.php";
     }
     public function addCategoryAction(){
		$appcategory = $this->cache->GetACache('appcategory');//读取缓存
		include  CURR_VIEW_PATH . "AppCategory/appcategory_add.php";
     }
	 public function doaddCategoryAction(){
		...
		$Data['packagename']   = ChkSQL(KS_G('post.packagename'));
		$Data['price']         = ChkSQL(KS_G('post.price'));
		$Data['market_price']  = ChkSQL(KS_G('post.market_price'));
		
		$Data['fonts']         = ChkSQL(KS_G('post.fonts'));
		$Data['intro']         = ChkSQL(KS_G('post.intro'));
		$CategoryModel         = new CategoryModel('common_app_categoryid');//实例化云服务表
		...
		KS_INFO('添加成功',0,M_URL('AppCategory'));
     }
	 ...
	 public function doeditCategoryAction(){ 
		...
		$this->cache->PutACache('appcategory');//存入缓存
		$url = M_URL('AppCategory','','',GP('p-'.$now_page));
		KS_INFO('修改成功',0,$url);
     }
	 ...
	 public function byOneAction(){
		...
		$page          = Page($options); 
		$appcategory   = $CategoryModel->getPage($page,'rootid,orderid','where parentid=0'); 
		include  CURR_VIEW_PATH . "AppCategory/appcategory_obyone.php";
     }
	 public function byOneUpAction(){
		...
		$this->cache->PutACache('appcategory');//存入缓存
		$url = M_URL('AppCategory','byOne','',GP('p-'.$now_page));
		KS_INFO('修改成功',0,$url);
     }
	 ...
	 public function byAllAction(){
		...
		include  CURR_VIEW_PATH . "AppCategory/appcategory_obyall.php";
     }
	 //向上移动
	 public function byAllUpAction(){
	    ...
	    $this->cache->PutACache('appcategory');//存入缓存
		$url = M_URL('AppCategory','byAll','',GP('p-'.$now_page));
		KS_INFO('修改成功',0,$url);
     }
	 //--------------------------------------向下移动----------------------------------------------//
