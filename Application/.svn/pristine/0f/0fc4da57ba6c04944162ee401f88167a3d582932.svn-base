...
<body class="ks-wrap">
	...
	<div class="Header-tool clearfix">
			...
			<div class="Tool-btnGroup">
				<a class="ks-bt bt-pur mr10" href="<?php echo M_URL('Offweb','showclassify'); ?>">分类管理</button></a>
			</div>
			<div class="Tool-btnGroup">
				<a class="ks-bt bt-pur mr10" href="<?php echo M_URL('Offweb','helpManage'); ?>"><i class="iconfont icon-jiahao"></i>添加帮助内容</a>
			</div>
			<!--按钮组件-->
			<!--搜索框组件-->
...
<form target="hidframe"  action="<?php echo M_URL('Offweb','deleteHelpCenter','',GP('p-'.$now_page));  ?>" method="post" id="myform">
...
<li>
	...
	<div class="ks-cbox clearfix">
		...
		<div class="fl ks-txtLeft">
			<p>标题：<?php echo Field($v['title']); ?></p>
			<p>时间：<?php echo date('Y-m-d',$v['adddate']); ?></p>
			<p>分类：<?php echo Field($v['categoryname']); ?></p>
		</div>
		<div class="ks-operaRight fr">
			<div class="ks-state" style="cursor: pointer;"></div>
			<ul class="clearfix">
				<li >
					<a href="<?php echo M_URL('Offweb','helpManage',$v['id'],GP('p-'.$now_page));?>">
						<i class="iconfont icon-bianji1"></i>
						<p class="class-name">编辑</p>
					</a>
				</li>
				<li>
					<a href="javascript:void(0);"  onclick="Confirm('是否删除？该操作不可逆!','<?php echo M_URL('Offweb','deleteHelpCenter',$v['id'],GP('p-'.$now_page)); ?>')">
