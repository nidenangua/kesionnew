...
<form target="hidframe" class="form-horizontal" action="<?php echo M_URL('Temp','AppletdoAdd',$id);?>" method="post" enctype="multipart/form-data">
						...
						<li>
							<div class="form-upload">
				          		<div class="js_uploadBox">
				          			<div class="js_showBox"><img class="js_logoBox block pic" onerror="this.src='//ks.kesion.com/Public/uploads/common/nopic.gif'" src="<?php if(!empty($value)){echo $value['pic'];} ?>"></div>
					                <div class="btn-upload clearfix">
					                    <a href="javascript:addTypeChange('<?php echo M_URL('Index','imageslist','',GP('type-top,name-pic',true));?>','930px','610px',{title:'选择图片',type:'top'});" class="selectImageBtn">
					                    	<span class="js_uploadText">上传图片</span></a>
					                    <p>限制：2MB,支持jpg|png|gif</p> 
					                    <input type="hidden" name="pic" value="<?php if(!empty($value)){echo $value['pic'];} ?>">
					               </div>
							    </div>
				          	</div>
						</li>
