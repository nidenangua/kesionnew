					...
					<ul>
						<li>
							...
							<div class="Created-Content clearfix">
								<input type="text" class="long-text" name="version" value="<?php if(!empty($res)){ echo $res['edition'];} ?>">
							</div>
						</li>
						<li>
							<h4>网址:</h4>
							<div class="Created-Content clearfix domain">
								<?php if(!empty($res)){ echo $res['downurl'];} ?>
							</div>
						</li>
						<li>
							<div class="form-upload">
				          		<div class="js_uploadBox">
					                <div class="btn-upload clearfix">
					                    <a href="javascript:addTypeChange('<?php echo M_URL('Index','padEdition','',GP('type-top,name-domain',true));?>','930px','610px',{title:'选择图片',type:'top'});" class="selectImageBtn">
					                    	<span class="js_uploadText">上传新安装包</span></a>
					                        <input type="hidden" name="domain" value="<?php if(!empty($res)){ echo $res['downurl'];} ?>">
					               </div>
							    </div>
				          	</div>
						</li>
