...
<meta name="viewport" content="width=device-width,initial-scale=1.0,minimum-scale=1.0,maximum-scale=1.0,user-scalable=no">
    <link rel="stylesheet" type="text/css" href="<?php echo UOOT;?>Public/app/css/login.css?<?php echoHash(); ?>" />
    <link rel="stylesheet" type="text/css" href="<?php echo UOOT;?>Public/app/css/base.css?<?php echoHash(); ?>" />
    <link rel="stylesheet" type="text/css" href="<?php echo UOOT;?>Public/app/css/page.animate.css?<?php echoHash(); ?>" />
...
<title>1分钟免费开通您的线上教育平台 - 科汛教育微门户</title>
</head>
	...
	<div class="login-center">
		...
		<div class="page-animate" data-delay="0.1" data-type="bounceInRight" >
		    <div class="login-content-r">
                <?php if (!isMobile()){?>
		        <p class="info-title">1分钟免费开通您的线上教育平台</p> 
                <?php } ?>
		        <div class="login-info clr">
		            ...
		            <ul class="clr reg-ul">
		            	<li class="clr button-wrp" style="display: none;">
		                    <div class="in-btn">
		                    <label class="teacher active"><input name="usertype" type="radio" value="2" <?php if(isset($users))echo "value='$users[usertype]'";?> checked>机构</label>	
		                    <label><input name="usertype" type="radio" value="1" <?php if(isset($users))echo "value='$users[usertype]'";?>>教师</label>
		                    </div>
		                </li>
		                <?php $base=$this->cache->GetACache('setting');if($base['regsitename']==1){?>
		            	<li>
		                ...
		                <li>
		                	...
		                	<label for="" class="realm-name">.<?php echo PLATDOMAIN;?></label>
		                </li>
		                <li>
		                	<div class="login-border">
						...
						<li class="cook clr">
		                    <div class="fl">
                                ...
                                <?php if (isMobile()){
                                ?>
    <input type="checkbox" name="" id="forRemember" value="" class="remember" checked="checked" />阅读并同意
                                <a href="<?php echo M_URL('Register','agreement');?>" class="forget">《服务协议》</a>
                                <?php
                                      }else{
                                ?>
                                <label for="forRemember" class="safe">
                                    <input type="checkbox" name="" id="forRemember" value="" class="remember" checked="checked" />
                                </label>
                                我已阅读并同意<a href="<?php echo M_URL('Register','agreement');?>" class="forget">《科汛V名师服务协议》</a>
                                 <?php } ?>
		                    </div>
		                </li>
		                <li>
		                	 <button type="button" onClick="do_reg();" class="login">立即注册</button>
							 <!-- <a class="now-reg login-reg" style="margin-top: 25px;" href="/login" >返回登录</a> -->
							 <div class="newsign">已有账号？<a href="/login">立即登录</a></div>
		                </li>
		            </ul>
		            </form>
		        </div>
