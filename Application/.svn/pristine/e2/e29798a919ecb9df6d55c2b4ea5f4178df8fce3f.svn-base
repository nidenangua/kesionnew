...
<div id="manage_top" class="menu_top_fixed  p15 height73">
    <div class="ibox-title">
        <h3 class="fl">
        		 <button type="button" onclick="addTypeChange('<?php echo M_URL('Setting','addresolution');?>','850','600px',{title:'新增分辨率',type:'top'});">添加</button>
        </h3>
        <div class="fr">
				...
				<?php foreach($values as $k=>$v){  ?>
					...
					<td>
						<div class="form-switch" open-value="1" >
							<label><input type="radio" name="isshuiyin<?php echo $k; ?>" value="1"  class="radioInput" 
								<?php if($v['isenable']==1){echo 'checked="checked"';} ?> 
									onclick='kaitong(<?php echo $v['wxid']; ?>);'>启用</label>
		           			<label><input type="radio" name="isshuiyin<?php echo $k; ?>" value="0"  
		           				<?php if($v['isenable']==0){echo 'checked="checked"';} ?>  
		           					 onclick="closes(<?php echo $v['wxid']; ?>);">不启用</label>
	           			</div>
...
<script>
	...
	function kaitong(wxid){

		$.ajax({
        type: "POST",
        url:"<?php echo M_URL('Setting','isSecretkey','',GP('')); ?>",
        data:{wxid:wxid,type:1},
	...
	function closes(wxid){
		alert(wxid);
		$.ajax({
        type: "POST",
        url:"<?php echo M_URL('Setting','isSecretkey','',GP('')); ?>",
        data:{wxid:wxid,type:2},
        success:function(text){
