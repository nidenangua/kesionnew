...
<style type="text/css">
	...
	.H5-gameList>li {position: relative;padding-bottom:20px;min-height: 120px; }
...
<div id="manage_top" class="menu_top_fixed  p15 height73" >
    <div class="ibox-title">
        <h3 class="fl">&nbsp;&nbsp;模板音乐
        	<a href="<?php echo M_URL('Temp','tempbgmAdd',$id);?>" class="btn btn-info btn-rounded btn-s-md  ml15" style="background: #FF5507;border: none;">添加音乐</a>
        ...
        <div class="fr">
            ...
            <input type="submit" class="sc-button" onclick="searchByClass('<?php echo GP(''); ?>','<?php echo M_URL('Temp','TempBgm',$id); ?>','<?php echo URL_MODEL;?>')" name="searchBtn" value="搜索">
      </form>
			<!--搜索-->
			</div>    
        </div>
		...
		<ul class="H5-gameOpt clearfix" style="padding-left: 10px;">
			...
			<li>
				<div class="shaixuan clearfix">
					<?php if($adddate == 2){?>
						<a class="desc" href="<?php echo M_URL('home/Temp','tempBgm',$id,GP("adddate-1")); ?>"><i class="desc-i"></i>添加时间</a>		
					<?php }else{ ?>
						<a class="desc" href="<?php echo M_URL('home/Temp','tempBgm',$id,GP("adddate-2")); ?>"><i class="asc-i"></i>添加时间</a>
					<?php }?>
...
<form target="hidframe" id="myform" method="post" target="hidframe"  action="<?php echo M_URL('home/Temp','deletebgms',$id,GP('p-'.$page->now_page.'option-'.$option)); ?>" class="form-horizontal" enctype="multipart/form-data">
		...
		<ul class="H5-gameList">
			...
			<li>
				<div class="card school-online">
						  	...
						  	<div class="card-body">
								<div class="clearfix">
									<div class="new-goup fl">
										<div class="new-cuo">
											<div>ID：<?php echo $v['id'];?> &nbsp;&nbsp;&nbsp;&nbsp;音乐名称：<?php echo $v['filename'];?></div>
											<div>上传时间：<span class="grayLing" style="margin: 0px;"><?php echo date('Y-m-d H:i:s',$v['adddate']); ?></span></div>
										</div>
									</div>
								</div>
							</div>
					...
					<div class="ring">&nbsp;&nbsp;
										<a class="grayLing" onclick="Confirm('是否删除此音乐','<?php echo M_URL('Temp','deletebgm',$v['id'],GP('p-'.$page->now_page.',tempid-'.$v['tempid']));?>');">删除</a>
						</div>
						<div class="ks-operaRight fr" >
		        		<ul class="clearfix">
		        			 <?php if($v['ism']==0){?> 
									 <li>
										 <a href="<?php echo M_URL('Temp','defaultBgm',$v['id'],GP('option-'.$option.',p-'.$page->now_page));?>">
			             ...
			             <?php }?>
										<li>
											<a href="<?php echo M_URL('Temp','bgmEdit',$v['id'],GP('p-'.$page->now_page));?>">
