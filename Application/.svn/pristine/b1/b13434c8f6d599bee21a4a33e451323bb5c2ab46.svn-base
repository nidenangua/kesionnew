...
<form target="hidframe" id="myform" class="form-horizontal" action="<?php if(empty($value)){echo M_URL('Label','LabeldoAdd','',GP('labeltype-'.$labeltype));}else{echo M_URL('Label','LabeldoEdit',$labelid,GP('p-'.$now_page));} ?>" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-4">
               <label class="control-label fl">{KS_</label>
                <div class="w150 fl">
                    <input class="form-control" type="text" value="<?php if(!empty($value)){echo $value['labelname'];} ?>" name="labelname"/>
                </div>
                <label class="control-label fl"> } * 调用格式：{KS_标签名称}</label>
            </div>
        </div>
        <div class="line line-dashed b-b line-lg pull-in"></div>
        <div class="form-group">
        <label class="col-sm-2 control-label">选择应用：</label>
			...
			<script>
				...
				{
					if(v==$("#appid")[0].options[i].value)
					  $("#appid")[0].options[i].selected=true;
				...
				if(appid!='0')
		        {
					$.ajax({
							...
							success:function(data){
								if(data!=-1)
								{
									$('#field').html('<li onclick="InsertLabel(\'userid\',\'-1\')">用户ID</li><li onclick="InsertLabel(\'username\',\'1\')">用户名</li><li onclick="InsertLabel(\'mobile\',\'1\')">用户手机号</li><li onclick="InsertLabel(\'mobile\',\'1\')">用户手机号</li><li onclick="InsertLabel(\'email\',\'1\')">用户邮箱</li><li onclick="InsertLabel(\'defaultpic\',\'1\')">用户头像</li>'+data);
								}
							}
						});
				}else{
					  $('#field').html('<li onclick="InsertLabel(\'userid\',\'-1\')">用户ID</li><li onclick="InsertLabel(\'username\',\'1\')">用户名</li><li onclick="InsertLabel(\'mobile\',\'1\')">用户手机号</li><li onclick="InsertLabel(\'mobile\',\'1\')">用户手机号</li><li onclick="InsertLabel(\'email\',\'1\')">用户邮箱</li><li onclick="InsertLabel(\'defaultpic\',\'1\')">用户头像</li>');
					 }
			}
			</script>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-8 taps-li">
              <ul id="field"><li onclick="InsertLabel('userid','-1')">用户ID</li><li onclick="InsertLabel('username','1')">用户名</li><li onclick="InsertLabel('mobile','1')">用户手机号</li><li onclick="InsertLabel('mobile','1')">用户手机号</li><li onclick="InsertLabel('email','1')">用户邮箱</li><li onclick="InsertLabel('defaultpic','1')">用户头像</li>
              <?php if(!empty($value)&&!empty($appid)){$common_table_field = M('common_table_field');$tableename = $this->cache->GetA('appinfo','appid',$appid,'tableename');$common_table = M('common_table');$tableid = $common_table->getOne('tableid','where tablename="common_user_'.$tableename.'" order by orderid limit 1');$values= $common_table_field->getAll('where tableid='.$tableid.' order by orderid');foreach($values as $k=>$v){?>
