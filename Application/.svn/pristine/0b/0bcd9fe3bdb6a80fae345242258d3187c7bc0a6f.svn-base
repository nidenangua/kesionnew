...
<form target="hidframe" id="myform" class="form-horizontal" action="<?php if(empty($value)){echo M_URL('Label','NavigationdoAdd','',GP('labeltype-'.$labeltype));}else{echo M_URL('Label','NavigationdoEdit',$labelid,GP('labeltype-'.$labeltype.',tempid-'.$tempid));} ?>" method="post" enctype="multipart/form-data">
