<template>
	<view>
		<view style="position: fixed;top:0;width: 100%;z-index: 1000;">
			<ks-tabs :value.sync="index" :tabList="tabList" @change="changeList" rangeKey="name"></ks-tabs>
		</view>
		
		<view class="order-list" style="margin-top: 54px;">
			<view class="order-item" v-for="(item,i) in list" :key="i" >
				...
				<block v-for="(site,s) in item.itemlist" :key="s">
					<view class="order-image" @click="open(site)">
				...
				<view class="item-bottom">
					<view style="text-align: right;margin-bottom: 5px;" v-if="item.use_couponmoney - 0 > 0">抵扣:<text style="color: #FF3B30;">￥{{item.use_couponmoney}}</text></view>
					<view style="text-align: right;">实付:<text style="color: #FF3B30;">{{item.pay_amount}}</text></view>
...
<script>
	...
	import {KsTabs} from 'components/ks-components/tabs/tabs'
