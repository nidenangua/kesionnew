<template>
	<view class="collect-page" :style="{minHeight:windowHeight+'px'}">
		<ks-tabs line :value.sync="index" :shadow="false" :scroll="true" range-key="name" :tab-list="tabList" @change="changeTabs" position="fixed"></ks-tabs>
	
		<view class="collect-list" v-if="status !== 'ask'">
			<view class="collect-item ks-line" v-for="(item,i) in list" :key="i" @click="open(item)">
		...
		<view class="collect-list" v-else>
			<view class="collect-item ks-line" v-for="(item,i) in list" :key="i" style="margin-top: 10px;" @click="openAsk(item)">
	...
	export default {
		...
		onLoad() {
			...
			this.$store.ready(state=> {
			  let model = state.userInfo.model
			  let modelList = [
			    {
			      value: -1,
			      name: "全部"
