	...
	<view style="background: #fff;">
		...
		<view class="tabs-header ks-line">
			<view class="tabs-header__item" @click="slideTabs(0)">
				...
				<image :src="baseUrl+'/static/images/choose-cade/choose-Cades.png?1.0'" v-if="activeTab==0"></image>
				<image :src="baseUrl+'/static/images/choose-cade/choose-Cadex.png?1.0'" v-else></image>
			</view>
			<view class="tabs-header__item" @click="slideTabs(1)">
				<text :class="activeTab == 1&&'active'">{{className}}</text>
				<image :src="baseUrl+'/static/images/choose-cade/choose-Cades.png?1.0'" v-if="activeTab==1"></image>
				<image :src="baseUrl+'/static/images/choose-cade/choose-Cadex.png?1.0'" v-else></image>
			</view>
		</view>
		<view :class="visible?'popup-box':'hide-box'"  @click="visible=false,this.activeTab=null"  @touchmove.stop >
			<view style="background: #fff;color:rgba(53,53,53,1);" v-if="activeTab == 0">
				<view class="flex-box__space type-item" :class="activeType === item.type&&'active'" v-for="(item,i) in typeList" :key="i" @click.stop="slideType(item.type)">
					<text>{{item.name}}</text>
					<image :src="baseUrl+'/static/images/choose-cade/lesson_icon_select@2x.png?1.0'" v-if="activeType == item.type"></image>
				</view>
			</view>
					...
					<view style="height: 70px;">
						...
						<view class="flex-box__space" style="font-size: 12px;margin-top: 10px;">
							<view class="flex-box" style="height:16px;">
								<uni-rate disabled="true" :value="Number(item.coursepf)" size="16" active-color="#2987ee"></uni-rate>
								<span style="color:#2987ee;margin-left:10px;">{{item.coursepf}}分</span>
							</view>
