		...
		<view class="ks-course__item" v-for="(item,i) in list" :key="i" @click="open(item)">			
			...
			<view>
				...
				<view v-if="isshowprice">
					<text class="ks-course__price">{{item.pricetext}}</text>
	...
	<view class="ks-course__list"   v-else-if="type == 2">
			...
			<view style="white-space: nowrap;overflow: auto;">
				...
				<view class="live-course__item" v-for="(item,i) in list" :key="i" @click="open(item)">
					<view>
						<view style="padding: 0 10px;display: flex;flex-direction: column;">
							<ks-image class="ks-image" radius="4px" :src="item.photo"></ks-image>
							<view class="ks-course__title--1" style="margin-top:5px">{{item.name}}</view>
						</view>
						    
						<view style="position: relative;background: #0096887d;width: 100%;height: 3px;margin: 15px 0;">
							<view style="width: 20px;height: 20px;background: #fff;border-radius: 50%;overflow: hidden;display: flex;justify-content: center;align-items: center;position: absolute;left: 50%;margin-left: -10px;top: -8.5px;">
								<view style="width: 14px;height: 14px;border-radius: 50%;overflow: hidden;background: #0096887d;"></view>
							</view>
						</view>
						<view style="text-align: center;">2019-12-16</view>
						<view style="display: flex;justify-content: space-between;font-size: 12px;align-items: center;padding: 0 10px;line-height: 20px;margin-top: 5px;">
							<view>讲师：{{item.teacher}}</view>
						    <view class="ks-course__price" style="font-size: 12px;" v-if="isshowprice">{{item.pricetext}}</view>
						    </view>
						</view>
					</view>    
				</view>
		...
		<view class="concentration-course__item" v-for="(item,i) in list" :key="i" @click="open(item)">
			...
			<view class="flex-box__space" style="font-size: 12px;">
				<view class="flex-box" style="height:16px;">
					<uni-rate disabled="true" :value="item.pjnum" size="14" active-color="#ff663d"></uni-rate>
					<text style="color:#ff663d;margin-left:10px;">{{item.pjnum}}分</text>
				</view>
				<view>{{item.count}}人学习</view>
			</view>
			<view class="ks-course__price" v-if="isshowprice">{{item.pricetext}}</view>
			</view>
		</view>
	</view>
